...
var initApp = (function(app) {
	...
	app.domReadyMisc = function() {
		...
		if (myapp_config.appDateHook.length) {
			var applang = localStorage.getItem('language') || 'en',
				now = new Date(),
				formatted = now.toLocaleString(applang, { weekday: "long" }) + ', ' +  
							now.toLocaleString(applang, { day: "numeric" }) + ' ' +  
							now.toLocaleString(applang, { month: "long" }) + ', ' + 
							now.getFullYear();
	...
	app.loadLanguage = function (applang) {
		if (!$.i18n) {
			//jQuery.getScript('http://url/to/the/script');
...
document.addEventListener('DOMContentLoaded', function() {
	...
	initApp.loadLanguage(localStorage.getItem('language') || 'en');
}); 
