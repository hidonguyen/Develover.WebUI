var kPIRegistration = (function (self) {
    let baseUrl = '/kpiregistration';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-kpi-registration");

    let isModal = false;
    if ($("#modal-kpi-registration").length) {
        context = $("#modal-kpi-registration");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-kpi-registrations').dataTable({
            responsive: true,
            ajax: baseUrl + '/getlist',
            columns: [
                { data: "divisionId", title: "Tổ" },
                { data: "kpiType", title: "Loại định mức" },
                { data: "issueDate", title: "Ngày đăng ký" }
            ],
            columnDefs: [
            ],
            dom:
    ...
    let loadData = () => {
        ...
        $("#kpiType", context).val(model.kPIType);
        $("#IssueDate", context).val(model.issueDate);
    }

    let createModel = () => {
        model.divisionId = $("select#DivisionId", context).val();
        model.kpiType = $("input#KPIType", context).val();
        model.issueDate = $("input#IssueDate", context).val();
    }
