function ContractType(options, model) {
    ...
    this.save = () => {
        ...
        $.ajax({
            ...
            swal({
                title: "Success",
                text: "The Contract Type has been saved!",
                icon: "success",
                ...
                if (options.modalId && postbackId && postbackId !== "" && postbackClass && postbackClass !== "") {
                    ...
                    window.location.href = "/catalog/detailcontracttype?id=" + res.id;
                }
            })
        }).fail((err) => {
            if (err.status === 400) {
    ...
    this.delete = () => {
        swal({
            ...
            text: "Do you want to delete the contract type now? This cannot be undone.",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        }).then((res) => {
            if (res) {
                $.ajax({
                    url: "/catalog/deletecontracttype",
                    type: "post",
                    data: { id: model.id }
                }).done((res) => {
                    swal({
                        title: "Success",
                        text: "The contract type has been deleted!",
                        icon: "success",
                        timer: develoverSettings.swal.timer,
                        closeOnClickOutside: develoverSettings.swal.closeOnClickOutside,
                        closeOnEsc: develoverSettings.swal.closeOnEsc
                    }).then(() => {
                        window.location.href = "/catalog/contracttypes";
                    })
                }).fail((err) => {
                    swal("[" + err.status + "] " + err.responseText, {
                        icon: "error",
