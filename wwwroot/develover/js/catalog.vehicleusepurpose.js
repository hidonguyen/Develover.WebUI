function VehicleUsePurpose(options, model) {
    ...
    this.save = () => {
        ...
        $.ajax({
            url: "/catalog/savevehicleusepurpose",
            type: "post",
            data: { mode, model }
        }).done((res) => {
            ...
            swal({
                title: "Success",
                text: "The Vehicle Use Purpose has been saved!",
                icon: "success",
                ...
                if (options.modalId && postbackId && postbackId !== "" && postbackClass && postbackClass !== "") {
                    ...
                    window.location.href = "/catalog/detailvehicleusepurpose?id=" + res.id;
                }
            })
        }).fail((err) => {
            if (err.status === 400) {
    ...
    this.delete = () => {
        swal({
            title: "Delete Vehicle Use Purpose?",
            text: "Do you want to delete the vehicle use Purpose now? This cannot be undone.",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        }).then((res) => {
            if (res) {
                $.ajax({
                    url: "/catalog/deletevehicleusepurpose",
                    type: "post",
                    data: { id: model.id }
                }).done((res) => {
                    swal({
                        title: "Success",
                        text: "The vehicle Use Purpose has been deleted!",
                        icon: "success",
                        timer: develoverSettings.swal.timer,
                        closeOnClickOutside: develoverSettings.swal.closeOnClickOutside,
                        closeOnEsc: develoverSettings.swal.closeOnEsc
                    }).then(() => {
                        window.location.href = "/catalog/vehicleusepurposes";
                    })
                }).fail((err) => {
                    swal("[" + err.status + "] " + err.responseText, {
                        icon: "error",
