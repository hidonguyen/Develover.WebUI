var thisVehicle = (function (self) {
    let baseUrl = '/vehicle';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-vehicle");

    let isModal = false;
    if ($("#modal-vehicle").length) {
        context = $("#modal-vehicle");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-vehicles').dataTable({
            responsive: true,
            ajax: baseUrl + '/getlist',
            columns: [
                { data: "registrationNo", title: "Sổ đăng ký" },
                { data: "registrationDate", title: "Ngày đăng ký" },
                { data: "manufactureDate", title: "Ngày sản xuất" },
                { data: "purchaseDate", title: "Ngày mua" },
                { data: "engineNo", title: "Động cơ" },
                { data: "chassisNo", title: "Khung gầm" },
                { data: "brand", title: "Nhãn hiệu" },
                { data: "modelNo", title: "Mẫu" },
                { data: "color", title: "Màu sắc" },
                { data: "capacity", title: "Sức chứa" },
                { data: "defaultDriverId", title: "Mã trình điều khiển mặc định" },
                { data: "defaultDepartmentId", title: "Mã bộ phận mặc định" },
    ...
    let loadData = () => {
        ...
        //binding to control
        $("#RegistrationNo", context).val(model.registrationNo);
        $("#RegistrationDate", context).val(model.registrationDate);
        $("#ManufactureDate", context).val(model.manufactureDate);
        $("#PurchaseDate", context).val(model.purchaseDate);
        $("#EngineNo", context).val(model.engineNo);
        $("#ChassisNo", context).val(model.chassisNo);
        $("#Brand", context).val(model.brand);
        $("#ModelNo", context).val(model.modelNo);
        $("#Color", context).val(model.color);
        $("#Capacity", context).val(model.capacity);
        $("#DefaultDriverId", context).val(model.defaultDriverId);
        $("#DefaultDepartmentId", context).val(model.defaultDepartmentId);
        $("#Note", context).val(model.note);
        $("#Status", context).prop('checked', model.status);
    }

    let createModel = () => {
        model.registrationNo = $("input#RegistrationNo", context).val();
        model.registrationDate = $("input#RegistrationDate", context).val();
        model.manufactureDate = $("input#ManufactureDate", context).val();
        model.purchaseDate = $("input#PurchaseDate", context).val();
        model.engineNo = $("input#EngineNo", context).val();
        model.chassisNo = $("input#ChassisNo", context).val();
        model.brand = $("input#Brand", context).val();
        model.modelNo = $("input#ModelNo", context).val();
        model.color = $("input#Color", context).val();
        model.capacity = $("input#Capacity", context).val();
        model.defaultDriverId = $("select#DefaultDriverId", context).val();
        model.defaultDepartmentId = $("select#DefaultDepartmentId", context).val();
