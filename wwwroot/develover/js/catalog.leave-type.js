var leaveType = (function (self) {
    let baseUrl = '/leavetype';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-leave-type");

    let isModal = false;
    if ($("#modal-leave-type").length) {
        context = $("#modal-leave-type");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-leave-types').dataTable({
            ...
            columnDefs: [
                {
                    targets: 0,
                    render: leaveTypeDrillDownFormatter
                },
                {
                    targets: -1,
                    width: 65,
