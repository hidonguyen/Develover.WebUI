function InboxDocument(options, model) {
    ...
    this.addAppendix = (closeAfterAdd) => {
        ...
        $.ajax({
            url: "/documents/inbox/validateappendixmodel",
            type: "post",
            data: { model: appendix }
        }).done((res) => {
    ...
    [TotalPages] = AutoNumeric.multiple(["input#TotalPages"], develoverSettings.OptionAutoNumericQuantity);
    ...
    function createModel() {
        ...
        model.refNo = $("input#RefNo", $form).val();
    ...
    this.save = () => {
        ...
        $.ajax({
            ...
            swal({
                ...
                window.location.href = "/documents/inbox/detail?id=" + res.id;
            })
        }).fail((err) => {
            if (err.status === 400) {
    ...
    this.initialize = () => {
        ...
        $form.find("input#RefNo").prop("disabled", true);
        createNo();
    }

    $('input#RefNo', $form).on('change', createNo);
