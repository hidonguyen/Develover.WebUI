var changePassword = (function (self) {
    let baseUrl = '/changepassword';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-change-password");

    let isModal = false;
    if ($("#modal-change-password").length) {
        context = $("#modal-change-password");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-change-passwords').dataTable({
            responsive: true,
            ajax: baseUrl + '/getlist',
            columns: [
                { data: "username", title: "Tên tài khoản" },
                { data: "password", title: "Mật khẩu" },
                { data: "confirmPassword", title: "Xác nhận mật khẩu" }
            ],
            columnDefs: [
                {
                    targets: -1,
    ...
    let loadData = () => {
        ...
        $("#Password", context).val(model.password);
        $("#ConfirmPassword", context).val(model.confirmPassword);
        $("#Status", context).prop('checked', model.status);
    }

    let createModel = () => {
        model.username = $("input#Username", context).val();
        model.password = $("input#Password", context).val();
        model.confirmPassword = $("input#ConfirmPassword", context).val();
