var documentType = (function (self) {
    let baseUrl = '/documenttype';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-document-type");

    let isModal = false;
    if ($("#modal-document-type").length) {
        context = $("#modal-document-type");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-document-types').dataTable({
            ...
            columnDefs: [
                {
                    targets: 0,
                    render: documentTypeDrillDownFormatter
                },
                {
                    targets: -1,
                    width: 65,
