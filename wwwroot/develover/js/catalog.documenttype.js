function DocumentType(options, model) {
    ...
    this.save = () => {
        ...
        $.ajax({
            ...
            swal({
                title: "Success",
                text: "The Document Type has been saved!",
                icon: "success",
                ...
                if (options.modalId && postbackId && postbackId !== "" && postbackClass && postbackClass !== "") {
                    ...
                    window.location.href = "/catalog/detaildocumenttype?id=" + res.id;
                }
            })
        }).fail((err) => {
            if (err.status === 400) {
    ...
    this.delete = () => {
        swal({
            title: "Delete Type?",
            text: "Do you want to delete the document type now? This cannot be undone.",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        }).then((res) => {
            if (res) {
                $.ajax({
                    url: "/catalog/deletedocumenttype",
                    type: "post",
                    data: { id: model.id }
                }).done((res) => {
                    swal({
                        ...
                        window.location.href = "/catalog/documenttypes";
                    })
                }).fail((err) => {
                    swal("[" + err.status + "] " + err.responseText, {
                        icon: "error",
