function InternalDocument(options, model) {
    ...
    this.addAppendix = (closeAfterAdd) => {
        ...
        $.ajax({
            url: "/documents/internal/validateappendixmodel",
            type: "post",
            data: { model: appendix }
        }).done((res) => {
    ...
    function createModel() {
        ...
        model.fromDepartmentId = $("select#FromDepartmentId", $form).val();
        model.deliverId = $("select#DeliverId", $form).val();
        model.toDepartmentId = $("select#ToDepartmentId", $form).val();
    ...
    this.save = () => {
        ...
        $.ajax({
            ...
            swal({
                ...
                window.location.href = "/documents/internal/detail?id=" + res.id;
            })
        }).fail((err) => {
            if (err.status === 400) {
            ...
            if (res) {
                $.ajax({
                    ...
                    swal({
                        ...
                        window.location.href = "/documents/internal";
                    })
                }).fail((err) => {
                    swal("[" + err.status + "] " + err.responseText, {
                        icon: "error",
    ...
    this.cancel = () => {
        swal({
            ...
            if (res) {
                $.ajax({
                    url: "/documents/internal/cancel",
                    type: "post",
                    data: { mode, model }
                }).done((res) => {
                    if (mode === DocumentMode.EDIT) {
                        window.location.href = "/documents/internal/detail?id=" + model.id;
                    }
                    else {
                        history.back();
                    }
    ...
    $('select#FromDepartmentId', $form).on('change', GetDocumentSeq);
    ...
    function GetDocumentSeq() {
        ...
        let departmentId = $('select#FromDepartmentId').val();

        if (!typeId || typeId == '' || issueDate == '' || !departmentId || departmentId == '')
            return;

        $.ajax({
            url: '/documents/internal/getdocumentseq',
            type: 'get',
