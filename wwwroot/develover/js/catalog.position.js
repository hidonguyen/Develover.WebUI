var position = (function (self) {
    let baseUrl = '/position';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-position");

    let isModal = false;
    if ($("#modal-position").length) {
        context = $("#modal-position");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-positions').dataTable({
            ...
            columnDefs: [
                {
                    targets: 0,
                    render: positionDrillDownFormatter
                },
                {
                    targets: -1,
                    width: 65,
