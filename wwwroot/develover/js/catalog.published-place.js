var publishedPlace = (function (self) {
    let baseUrl = '/publishedplace';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-published-place");

    let isModal = false;
    if ($("#modal-published-place").length) {
        context = $("#modal-published-place");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-published-places').dataTable({
            ...
            columnDefs: [
                {
                    targets: 0,
                    render: publishedPlaceDrillDownFormatter
                },
                {
                    targets: -1,
                    width: 65,
