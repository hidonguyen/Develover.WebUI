var departmentTransfer = (function (self) {
    let baseUrl = '/departmenttransfer';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-department-transfer");

    let isModal = false;
    if ($("#modal-department-transfer").length) {
        context = $("#modal-department-transfer");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-department-transfers').dataTable({
            responsive: true,
            ajax: baseUrl + '/getlist',
            columns: [
                { data: "employeeId", title: "Nhân viên" },
                { data: "no", title: "Số quyết định" },
                { data: "effectiveDate", title: "Ngày hiệu lực" },
                { data: "divisionId", title: "Tổ" },
    ...
    let loadData = () => {
        ...
        //binding to control
        $("#EmployeeId", context).val(model.employeeId);
        $("#No", context).val(model.no);
        $("#EffectiveDate", context).val(model.effectiveDate);
        $("#DivisionId", context).val(model.divisionId);
        $("#Note", context).val(model.note);
        $("#Status", context).prop('checked', model.status);
    }

    let createModel = () => {
        model.employeeId = $("select#EmployeeId", context).val();
        model.no = $("input#No", context).val();
        model.effectiveDate = $("input#EffectiveDate", context).val();
        model.divisionId = $("select#DivisionId", context).val();
