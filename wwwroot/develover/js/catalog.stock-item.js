function StockItem(options, model) {
    ...
    function createModel() {
        ...
        model.defaultLocationId = $("select#DefaultLocationId", $form).val();
        model.unitOfMeasureId = $("select#UnitOfMeasureId", $form).val();
        model.status = $("input#Status", $form).prop("checked");
    }

    this.save = () => {

        if (!validateDataInput(["Name", "UnitOfMeasureId"], ["Name", "Unit of Measure"], $form))
            return;

        createModel();

        $.ajax({
            url: "/catalog/savestockitem",
            type: "post",
            data: { mode, model }
        }).done((res) => {
            ...
            swal({
                title: "Success",
                text: "The stock item has been saved!",
                icon: "success",
                ...
                if (options.modalId && postbackId && postbackId !== "" && postbackClass && postbackClass !== "") {
                    ...
                    window.location.href = "/catalog/detailstockitem?id=" + res.id;
                }
            })
        }).fail((err) => {
            if (err.status === 400) {
    ...
    this.delete = () => {
        swal({
            ...
            text: "Do you want to delete the stock item now? This cannot be undone.",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        }).then((res) => {
            if (res) {
                $.ajax({
                    ...
                    swal({
                        ...
                        window.location.href = "/catalog/stockitems";
                    })
                }).fail((err) => {
                    swal("[" + err.status + "] " + err.responseText, {
                        icon: "error",
