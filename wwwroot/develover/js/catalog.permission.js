var permission = (function (self) {
    let baseUrl = '/permission';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-permission");

    let isModal = false;
    if ($("#modal-permission").length) {
        context = $("#modal-permission");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-permissions').dataTable({
            ...
            columnDefs: [
                {
                    targets: 0,
                    render: permissionDrillDownFormatter
                },
                {
                    targets: -1,
                    width: 65,
