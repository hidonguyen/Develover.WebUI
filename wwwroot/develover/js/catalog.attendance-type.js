var attendanceType = (function (self) {
    let baseUrl = '/attendancetype';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-attendance-type");

    let isModal = false;
    if ($("#modal-attendance-type").length) {
        context = $("#modal-attendance-type");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-attendance-types').dataTable({
            responsive: true,
            ajax: baseUrl + '/getlist',
            columns: [
                { data: "name", title: "Tên" },
                { data: "multiplier", title: "Hệ số nhân" },
                { data: "hasMealAllowance", title: "Có phụ cấp cơm" },
                { data: "isAnnualLeave", title: "Tính nghỉ phép năm" },
                { data: "status", title: "Tình trạng" }
            ],
            columnDefs: [
                {
                    targets: 0,
                    render: attendanceTypeDrillDownFormatter
                },
                {
                    targets: -1,
                    width: 65,
    ...
    let loadData = () => {
        ...
        $("#Multiplier", context).val(model.multiplier);
        $("#HasMealAllowance", context).val(model.hasMealAllowance);
        $("#IsAnnualLeave", context).val(model.isAnnualLeave);
        $("#Status", context).prop('checked', model.status);
    }

    let createModel = () => {
        model.name = $("input#Name", context).val();
        model.multiplier = $("input#Multiplier", context).val();
        model.hasMealAllowance = $("input#HasMealAllowance", context).val();
        model.isAnnualLeave = $("input#IsAnnualLeave", context).val();
