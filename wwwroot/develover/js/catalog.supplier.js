function Supplier(options, model) {
    ...
    function createModel() {
        ...
        model.taxCode = $("input#TaxCode", $form).val();
    ...
    this.save = () => {
        ...
        $.ajax({
            ...
            swal({
                title: "Success",
                text: "The Supplier has been saved!",
                icon: "success",
                ...
                if (options.modalId && postbackId && postbackId !== "" && postbackClass && postbackClass !== "") {
                    ...
                    window.location.href = "/catalog/detailsupplier?id=" + res.id;
                }
            })
        }).fail((err) => {
            if (err.status === 400) {
    ...
    this.delete = () => {
        swal({
            ...
            text: "Do you want to delete the supplier now? This cannot be undone.",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        }).then((res) => {
            if (res) {
                $.ajax({
                    ...
                    swal({
                        ...
                        window.location.href = "/catalog/suppliers";
                    })
                }).fail((err) => {
                    swal("[" + err.status + "] " + err.responseText, {
                        icon: "error",
