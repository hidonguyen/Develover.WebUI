var division = (function (self) {
    let baseUrl = '/division';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-division");

    let isModal = false;
    if ($("#modal-division").length) {
        context = $("#modal-division");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-divisions').dataTable({
            ...
            columnDefs: [
                {
                    targets: 0,
                    render: divisionDrillDownFormatter
                },
                {
                    targets: -1,
                    width: 65,
    ...
    let createModel = () => {
        ...
        model.departmentId = $("select#DepartmentId", context).val();
