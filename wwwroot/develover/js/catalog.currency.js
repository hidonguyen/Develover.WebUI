var currency = (function (self) {
    let baseUrl = '/currency';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-currency");

    let isModal = false;
    if ($("#modal-currency").length) {
        context = $("#modal-currency");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-currencys').dataTable({
            responsive: true,
            ajax: baseUrl + '/getlist',
            columns: [
                { data: "name", title: "Tên" },
                { data: "exchangeRate", title: "Tỷ giá" },
            ...
            columnDefs: [
                {
                    targets: 0,
                    render: currencyDrillDownFormatter
                },
                {
                    targets: -1,
                    width: 65,
    ...
    let loadData = () => {
        ...
        $("#ExchangeRate", context).val(model.exchangeRate);
    ...
    let createModel = () => {
        ...
        model.exchangeRate = $("input#ExchangeRate", context).val();
