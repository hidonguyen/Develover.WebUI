var documentCategory = (function (self) {
    let baseUrl = '/documentcategory';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-document-category");

    let isModal = false;
    if ($("#modal-document-category").length) {
        context = $("#modal-document-category");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-document-categorys').dataTable({
            responsive: true,
            ...
            columnDefs: [
                {
                    targets: 0,
                    render: documentCategoryDrillDownFormatter
                },
                {
                    targets: -1,
                    width: 65,
