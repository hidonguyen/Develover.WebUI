var kpiType = (function (self) {
    let baseUrl = '/kpitype';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-kpi-type");

    let isModal = false;
    if ($("#modal-kpi-type").length) {
        context = $("#modal-kpi-type");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-kpi-types').dataTable({
            responsive: true,
            ajax: baseUrl + '/getlist',
            columns: [
                { data: "effectiveDate", title: "Ngày hiệu lực" },
                { data: "value0", title: "Loại 0" },
                { data: "value1", title: "Loại 1" },
                { data: "value2", title: "Loại 2" },
                { data: "value3", title: "Loại 3" },
                { data: "value4", title: "Loại 4" },
                { data: "value5", title: "Loại 5" },
            ],
            columnDefs: [
            ],
            dom:
    ...
    let loadData = () => {
        ...
        //binding to control
        $("#EffectiveDate", context).val(model.effectiveDate);
        $("#Value0", context).val(model.value0);
        $("#Value1", context).val(model.value1);
        $("#Value2", context).val(model.value2);
        $("#Value3", context).val(model.value3);
        $("#Value4", context).val(model.value4);
        $("#Value5", context).val(model.value5);
    }

    let createModel = () => {
        model.effectiveDate = $("input#EffectiveDate", context).val();
        model.value0 = $("input#Value0", context).val();
        model.value1 = $("input#Value1", context).val();
        model.value2 = $("input#Value2", context).val();
        model.value3 = $("input#Value3", context).val();
        model.value4 = $("input#Value4", context).val();
        model.value5 = $("input#Value5", context).val();
    }
