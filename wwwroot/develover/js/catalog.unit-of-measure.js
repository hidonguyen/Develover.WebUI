var unitOfMeasure = (function (self) {
    let baseUrl = '/unitofmeasure';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-unit-of-measure");

    let isModal = false;
    if ($("#modal-unit-of-measure").length) {
        context = $("#modal-unit-of-measure");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-unit-of-measures').dataTable({
            ...
            columnDefs: [
                {
                    targets: 0,
                    render: unitOfMeasureDrillDownFormatter
                },
                {
                    targets: -1,
                    width: 65,
