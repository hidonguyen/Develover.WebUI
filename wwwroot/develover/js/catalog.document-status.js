var documentStatus = (function (self) {
    let baseUrl = '/documentstatus';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-document-status");

    let isModal = false;
    if ($("#modal-document-status").length) {
        context = $("#modal-document-status");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-document-statuss').dataTable({
            ...
            columnDefs: [
                {
                    targets: 0,
                    render: documentStatusDrillDownFormatter
                },
                {
                    targets: -1,
                    width: 65,
