var allowanceDeductionType = (function (self) {
    let baseUrl = '/allowancedeductiontype';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-allowance-deduction-type");

    let isModal = false;
    if ($("#modal-allowance-deduction-type").length) {
        context = $("#modal-allowance-deduction-type");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-allowance-deduction-types').dataTable({
            responsive: true,
            ajax: baseUrl + '/getlist',
            columns: [
                { data: "name", title: "Tên" },
                { data: "showOnPayslip", title: "Giảm trừ" },
                { data: "isDeduction", title: "Hiện lên bảng lương" },
    ...
    let loadData = () => {
        ...
        $("#IsDeduction", context).prop('checked',model.isDeduction);
        $("#ShowOnPayslip", context).prop('checked',model.showOnPayslip);
        $("#Status", context).prop('checked', model.status);
    }

    let createModel = () => {
        model.name = $("input#Name", context).val();
        model.isDeduction = $("input#IsDeduction", context).prop("checked");
        model.showOnPayslip= $("input#ShowOnPayslip", context).prop("checked");
...
document.addEventListener('DOMContentLoaded', function () {
    allowanceDeductionType.initialize();
}); 