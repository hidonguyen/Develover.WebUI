function Role(options, model) {
        ...
        $.ajax({
            ...
            swal({
                ...
                if (options.modalId && postbackId && postbackId !== "" && postbackClass && postbackClass !== "") {
                    ...
                    window.location.href = "/identity/detailrole?id=" + res.id;
                }
            })
        }).fail((err) => {
            if (err.status === 400) {
    ...
    this.delete = () => {
        swal({
            title: "Delete Role?",
            text: "Do you want to delete the role now? This cannot be undone.",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        }).then((res) => {
            if (res) {
                $.ajax({
                    ...
                    swal({
                        ...
                        window.location.href = "/identity/roles";
                    })
                }).fail((err) => {
                    swal("[" + err.status + "] " + err.responseText, {
                        icon: "error",
