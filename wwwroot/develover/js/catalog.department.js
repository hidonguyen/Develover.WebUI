var department = (function (self) {
    let baseUrl = '/department';
    let mode = CatalogMode.VIEW;
    let modelId = urlParams.get('id');
    let context = $("#panel-department");

    let isModal = false;
    if ($("#modal-department").length) {
        context = $("#modal-department");
        isModal = true;
    }
    ...
    let initializeIndex = () => {
        $('#dt-departments').dataTable({
            ...
            columnDefs: [
                {
                    targets: 0,
                    render: departmentDrillDownFormatter
                },
                {
                    targets: -1,
                    width: 65,
