function Department(options, model) {
        ...
        $.ajax({
            ...
            swal({
                ...
                if (options.modalId && postbackId && postbackId !== "" && postbackClass && postbackClass !== "") {
                    ...
                    window.location.href = "/catalog/detaildepartment?id=" + res.id;
                }
            })
        }).fail((err) => {
            if (err.status === 400) {
    ...
    this.delete = () => {
        swal({
            ...
            text: "Do you want to delete the department now? This cannot be undone.",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        }).then((res) => {
            if (res) {
                $.ajax({
                    ...
                    swal({
                        ...
                        window.location.href = "/catalog/departments";
                    })
                }).fail((err) => {
                    swal("[" + err.status + "] " + err.responseText, {
                        icon: "error",
